 ...
 *
 */
public class CyclicSortArrayOfObjectsV3 {
  public static void sort(int[] nums) {
    for (int i = 0; i < nums.length; i++) {
      int j = i;
      while (nums[j] != j + 1 && j < nums.length) {
        int ith_val = nums[j];
        int partner_val = nums[ith_val - 1];
        System.out.println("Pre swap: i:" + j + ", ith_val=" + ith_val + ", partner_val=" + partner_val);

        nums[j] = partner_val;
        nums[partner_val - 1] = ith_val;
        System.out.println("Post swap: i:" + j + ", ith_val=" + nums[j] + ", partner_val=" + nums[partner_val - 1]);
        ++j;
      }
    }
  }
